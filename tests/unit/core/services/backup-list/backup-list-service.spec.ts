...
describe('Given a backup list service', () => {
  ...
  describe('Given list', () => {
    ...
    it('Should keep only backups of the exact database', async () => {
      const own = backup(ownKey);
      const other = backup(
        `${databaseConnection.name}-staging-2026-09-24T12-17-03Z.sql.gz`,
      );

      s3.list.mockResolvedValueOnce({ok: true, objects: [own, other]});
    ...
    it('Should return the database error', async () => {
      ...
      expect(await service.list()).toEqual({ok: false, error});
    });
  ...
  describe('Given latest', () => {
    it('Should return the newest key as latest', async () => {
      s3.list.mockResolvedValueOnce({
        ok: true,
        objects: [
    ...
    it('Should fail latest when there is no backup', async () => {
      s3.list.mockResolvedValueOnce({ok: true, objects: []});
    ...
    it('Should return the S3 error from latest', async () => {
      const error = new Error(faker.lorem.sentence());
